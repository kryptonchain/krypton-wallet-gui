...
import (
	"Krypton-Nest/turtlecoinwalletdrpcgo"
	"bufio"
	"io"
	"math/rand"
	"os"
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string) (transactionHash string, err error) {
	...
	if !strings.HasPrefix(transferAddress, "QQQ") || (len(transferAddress) != 98 && len(transferAddress) != 187) {
	...
	if transferAmount <= 0 {
		return "", errors.New("amount of ZOD to be sent should be greater than 0")
	}
...
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, useCheckpoints bool, daemonAddress string, daemonPort string) (err error) {
	...
	if isPlatformDarwin {
		...
		pathToAppLibDir := pathToHomeDir + "/Library/Application Support/Krypton-Nest"
